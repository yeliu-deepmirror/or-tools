...
class HIGHSInformation {
 public:
  explicit HIGHSInformation(bool maximize) : num_all_nodes_(0) {
...
// Function to be called in the HIGHS callback
void HIGHSGatherInformationCallback(glp_tree* tree, void* info) {
  CHECK(tree != nullptr);
  CHECK(info != nullptr);
  HIGHSInformation* highs_info = reinterpret_cast<HIGHSInformation*>(info);
  switch (glp_ios_reason(tree)) {
    // The best bound and the number of nodes change only when HIGHS
    // branches, generates cuts or finds an integer solution.
    case GLP_ISELECT:
    case GLP_IROWGEN:
    case GLP_IBINGO: {
      // Get total number of nodes
      glp_ios_tree_size(tree, nullptr, nullptr, &highs_info->num_all_nodes_);
      // Get best bound
      int node_id = glp_ios_best_node(tree);
      if (node_id > 0) {
        highs_info->best_objective_bound_ = glp_ios_node_bound(tree, node_id);
      }
      break;
    }
    default:
...
namespace {
// HIGHS indexes its variables and constraints starting at 1.
int MPSolverIndexToHighsIndex(int index) { return index + 1; }
}  // namespace

class HIGHSInterface : public MPSolverInterface {
 public:
  // Constructor that takes a name for the underlying highs solver.
  HIGHSInterface(MPSolver* const solver, bool mip);
  ...
  std::string SolverVersion() const override {
    return absl::StrFormat("HIGHS %s", glp_version());
  }
 ...
 private:
  ...
  void ConfigureHIGHSParameters(const MPSolverParameters& param);
  ...
  void ExtractOneConstraint(MPConstraint* const constraint, int* const indices,
                            double* const coefs);
  // Transforms basis status from HIGHS integer code to MPSolver::BasisStatus.
  MPSolver::BasisStatus TransformHIGHSBasisStatus(int highs_basis_status) const;
  ...
  glp_iocp mip_param_;
  // For the callback
  std::unique_ptr<HIGHSInformation> mip_callback_info_;
};

// Creates a LP/MIP instance with the specified name and minimization objective.
HIGHSInterface::HIGHSInterface(MPSolver* const solver, bool mip)
    : MPSolverInterface(solver), lp_(nullptr), mip_(mip) {
  // Make sure glp_free_env() is called at the exit of the current thread.
  SetupHighsEnvAutomaticDeletion();

  lp_ = glp_create_prob();
  glp_set_prob_name(lp_, solver_->name_.c_str());
  glp_set_obj_dir(lp_, GLP_MIN);
  mip_callback_info_ = std::make_unique<HIGHSInformation>(maximize_);
}
...
void HIGHSInterface::SetVariableBounds(int mpsolver_var_index, double lb,
                                      double ub) {
  InvalidateSolutionSynchronization();
  ...
  const int highs_var_index = MPSolverIndexToHighsIndex(mpsolver_var_index);
  if (lb != -infinity) {
    if (ub != infinity) {
      if (lb == ub) {
        glp_set_col_bnds(lp_, highs_var_index, GLP_FX, lb, ub);
      } else {
        glp_set_col_bnds(lp_, highs_var_index, GLP_DB, lb, ub);
      }
    } else {
      glp_set_col_bnds(lp_, highs_var_index, GLP_LO, lb, 0.0);
    }
  } else if (ub != infinity) {
    glp_set_col_bnds(lp_, highs_var_index, GLP_UP, 0.0, ub);
  } else {
    glp_set_col_bnds(lp_, highs_var_index, GLP_FR, 0.0, 0.0);
  }
}

void HIGHSInterface::SetVariableInteger(int mpsolver_var_index, bool integer) {
  InvalidateSolutionSynchronization();
  if (mip_) {
    if (variable_is_extracted(mpsolver_var_index)) {
      // Not cached if the variable has been extracted.
      glp_set_col_kind(lp_, MPSolverIndexToHighsIndex(mpsolver_var_index),
...
void HIGHSInterface::SetConstraintBounds(int mpsolver_constraint_index,
                                        double lb, double ub) {
  InvalidateSolutionSynchronization();
  ...
  const int highs_constraint_index =
      MPSolverIndexToHighsIndex(mpsolver_constraint_index);
  DCHECK(lp_ != nullptr);
  const double infinity = solver_->infinity();
  if (lb != -infinity) {
    if (ub != infinity) {
      if (lb == ub) {
        glp_set_row_bnds(lp_, highs_constraint_index, GLP_FX, lb, ub);
      } else {
        glp_set_row_bnds(lp_, highs_constraint_index, GLP_DB, lb, ub);
      }
    } else {
      glp_set_row_bnds(lp_, highs_constraint_index, GLP_LO, lb, 0.0);
    }
  } else if (ub != infinity) {
    glp_set_row_bnds(lp_, highs_constraint_index, GLP_UP, 0.0, ub);
  } else {
    glp_set_row_bnds(lp_, highs_constraint_index, GLP_FR, 0.0, 0.0);
  }
}
...
void HIGHSInterface::SetCoefficient(MPConstraint* const constraint,
  ...
  InvalidateSolutionSynchronization();
  // HIGHS does not allow to modify one coefficient at a time, so we
...
void HIGHSInterface::ClearConstraint(MPConstraint* const constraint) {
  ...
  if (constraint_is_extracted(constraint->index())) {
    glp_set_mat_row(lp_, MPSolverIndexToHighsIndex(constraint->index()), 0,
                    nullptr, nullptr);
  }
}
...
void HIGHSInterface::ClearObjective() {
  InvalidateSolutionSynchronization();
  for (const auto& entry : solver_->objective_->coefficients_) {
    ...
    if (!variable_is_extracted(mpsolver_var_index)) {
      ...
      glp_set_obj_coef(lp_, MPSolverIndexToHighsIndex(mpsolver_var_index), 0.0);
    }
  }
  // Constant term.
...
void HIGHSInterface::ExtractNewVariables() {
  ...
  if (total_num_vars > last_variable_index_) {
    ...
    for (int j = last_variable_index_; j < solver_->variables_.size(); ++j) {
      ...
      if (!var->name().empty()) {
        glp_set_col_name(lp_, MPSolverIndexToHighsIndex(j), var->name().c_str());
      }
      ...
      glp_set_obj_coef(lp_, MPSolverIndexToHighsIndex(j), tmp_obj_coef);
    }
...
void HIGHSInterface::ExtractOldConstraints() {
...
// coefficients.
void HIGHSInterface::ExtractOneConstraint(MPConstraint* const constraint,
                                         int* const indices,
                                         double* const coefs) {
  // HIGHS convention is to start indexing at 1.
  int k = 1;
  for (const auto& entry : constraint->coefficients_) {
    DCHECK(variable_is_extracted(entry.first->index()));
    indices[k] = MPSolverIndexToHighsIndex(entry.first->index());
    coefs[k] = entry.second;
    ++k;
  }
  glp_set_mat_row(lp_, MPSolverIndexToHighsIndex(constraint->index()), k - 1,
                  indices, coefs);
}

// Define new constraints on old and new variables.
void HIGHSInterface::ExtractNewConstraints() {
  ...
  if (last_constraint_index_ < total_num_rows) {
    ...
    for (int i = last_constraint_index_; i < total_num_rows; ++i) {
      ...
      if (ct->name().empty()) {
        glp_set_row_name(lp_, MPSolverIndexToHighsIndex(i),
                         absl::StrFormat("ct_%i", i).c_str());
      } else {
        glp_set_row_name(lp_, MPSolverIndexToHighsIndex(i), ct->name().c_str());
      }
    ...
    if (last_variable_index_ == 0 && last_constraint_index_ == 0) {
      ...
      for (int i = 0; i < solver_->constraints_.size(); ++i) {
        ...
        for (const auto& entry : ct->coefficients_) {
          DCHECK(variable_is_extracted(entry.first->index()));
          constraint_indices[k] = MPSolverIndexToHighsIndex(ct->index());
          variable_indices[k] = MPSolverIndexToHighsIndex(entry.first->index());
          coefs[k] = entry.second;
          ++k;
        }
      }
...
void HIGHSInterface::ExtractObjective() {
  // Linear objective: set objective coefficients for all variables
  // (some might have been modified).
  for (const auto& entry : solver_->objective_->coefficients_) {
    glp_set_obj_coef(lp_, MPSolverIndexToHighsIndex(entry.first->index()),
                     entry.second);
  }
  // Constant term.
...
MPSolver::ResultStatus HIGHSInterface::Solve(const MPSolverParameters& param) {
  ...
  // Note that HIGHS provides incrementality for LP but not for MIP.
  ...
  ConfigureHIGHSParameters(param);
  ...
  VLOG(1) << absl::StrFormat("HIGHS Status: %i (time spent: %.3f seconds).",
  ...
  for (int i = 0; i < solver_->variables_.size(); ++i) {
    ...
    double val;
    if (mip_) {
      val = glp_mip_col_val(lp_, MPSolverIndexToHighsIndex(i));
    } else {
      val = glp_get_col_prim(lp_, MPSolverIndexToHighsIndex(i));
    }
    var->set_solution_value(val);
    VLOG(3) << var->name() << ": value =" << val;
    if (!mip_) {
      double reduced_cost;
      reduced_cost = glp_get_col_dual(lp_, MPSolverIndexToHighsIndex(i));
  ...
  for (int i = 0; i < solver_->constraints_.size(); ++i) {
    ...
    if (!mip_) {
      const double dual_value =
          glp_get_row_dual(lp_, MPSolverIndexToHighsIndex(i));
      ct->set_dual_value(dual_value);
      VLOG(4) << "row " << MPSolverIndexToHighsIndex(i)
  ...
  if (mip_) {
    ...
    VLOG(1) << "HIGHS result status: " << tmp_status;
    if (tmp_status == GLP_OPT) {
      result_status_ = MPSolver::OPTIMAL;
    } else if (tmp_status == GLP_FEAS) {
      result_status_ = MPSolver::FEASIBLE;
    } else if (tmp_status == GLP_NOFEAS) {
      // For infeasible problems, HIGHS actually seems to return
      // GLP_UNDEF. So this is never (?) reached.  Return infeasible
      // in case HIGHS returns a correct status in future versions.
      result_status_ = MPSolver::INFEASIBLE;
    } else if (solver_status == GLP_ETMLIM) {
      result_status_ = MPSolver::NOT_SOLVED;
    } else {
      result_status_ = MPSolver::ABNORMAL;
      // HIGHS does not have a status code for unbounded MIP models, so
    ...
    if (tmp_status == GLP_OPT) {
      ...
      // For unbounded problems, HIGHS actually seems to return
...
MPSolver::BasisStatus HIGHSInterface::TransformHIGHSBasisStatus(
    int highs_basis_status) const {
  switch (highs_basis_status) {
    ...
    default:
      LOG(FATAL) << "Unknown HIGHS basis status";
      return MPSolver::FREE;
  }
}
...
MPSolver::BasisStatus HIGHSInterface::row_status(int constraint_index) const {
  ...
  const int highs_basis_status =
      glp_get_row_stat(lp_, MPSolverIndexToHighsIndex(constraint_index));
  return TransformHIGHSBasisStatus(highs_basis_status);
}
...
MPSolver::BasisStatus HIGHSInterface::column_status(int variable_index) const {
  ...
  const int highs_basis_status =
      glp_get_col_stat(lp_, MPSolverIndexToHighsIndex(variable_index));
  return TransformHIGHSBasisStatus(highs_basis_status);
}

bool HIGHSInterface::CheckSolutionExists() const {
  if (result_status_ == MPSolver::ABNORMAL) {
    LOG(WARNING) << "Ignoring ABNORMAL status from HIGHS: This status may or may"
...
double HIGHSInterface::ComputeExactConditionNumber() const {
  if (!IsContinuous()) {
  ...
  // HIGHS, so when a solution exists, a basis exists.
  CheckSolutionExists();
  const int num_rows = glp_get_num_rows(lp_);
  const int num_cols = glp_get_num_cols(lp_);
  // HIGHS indexes everything starting from 1 instead of 0.
...
double HIGHSInterface::ComputeScaledBasisL1Norm(
  ...
  for (int col = 1; col <= num_cols; ++col) {
    const int highs_basis_status = glp_get_col_stat(lp_, col);
    // Take into account only basic columns.
    if (highs_basis_status == GLP_BS) {
  ...
  // Slack variables.
  for (int row = 1; row <= num_rows; ++row) {
    const int highs_basis_status = glp_get_row_stat(lp_, row);
...
double HIGHSInterface::ComputeInverseScaledBasisL1Norm(
    int num_rows, int num_cols, double* row_scaling_factor,
    double* column_scaling_factor) const {
...
void HIGHSInterface::ConfigureHIGHSParameters(const MPSolverParameters& param) {
  if (mip_) {
    ...
    mip_param_.cb_func = HIGHSGatherInformationCallback;
...
void HIGHSInterface::SetDualTolerance(double value) { lp_param_.tol_dj = value; }
